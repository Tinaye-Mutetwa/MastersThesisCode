---
title: "qRT-PCR ISG Expression in IAV Infected IFNAR1 or IFNLR1 KO Cells"
subtitle: 'Thesis_Code_3: Figures 10'
output: 
  html_document:
    keep_md: true
---
...
g_all <- function(type,l,setMaxMin=T){
...
q<-df[df$Treatment %in% l,]  %>%
  ...
  scale_fill_manual(values=c("IFN-/IAV-"="grey", "IFN-/IAV+"="black","IFNB+/IAV-"="red","IFNB+/IAV+"="Orange", "IFNL+/IAV-"="Blue", "IFNL+/IAV+"="Purple"), breaks=treatment)+
...
### Heatmap Summary of ISG Expression in IAV infected or IFN treated IFN receptor knockout cells
...
d_long<-Reduce(function(x, y) merge(x, y, all=TRUE), list(dfM.b, dfIFNA.b, dfIFNb.b,dfIL29.b,dfIl28a.b,dfMXA.b,dfISG15.b,dfIFIT2.b))
...
ha = HeatmapAnnotation(df = df, gap = unit(c(1, 1), "mm"), col = list(Treatment = c("IFN-/IAV-"="gray", "IFN-/IAV+"="black","IFNB+/IAV-"="lightcoral","IFNB+/IAV+"="red", "IFNL+/IAV-"="skyblue1", "IFNL+/IAV+"="blue4"),
                                           Time = c("12"="darkseagreen4","24"= "darkgreen")),
...
Heatmap(W.matrix,show_column_dend = FALSE, col = f1,show_column_names=F,show_row_names = T, cluster_col=F, cluster_rows = F, heatmap_legend_param = list(title = ""),top_annotation = ha, rect_gp = gpar(type = "none"), cell_fun=cell_fun, column_title = "Wildtype", column_title_gp = gpar(fontsize = 20, fontface = "bold"))
```

```{r}
f2 = colorRamp2(seq(min(A.matrix), max(A.matrix), length = 3), c("#3c17fe", "#EEEEEE", "#b2312c"))
Heatmap(A.matrix,show_column_dend = FALSE, col = f2,show_column_names=F,show_row_names = T, cluster_col=F, cluster_rows = F, heatmap_legend_param = list(title = ""),top_annotation = ha, rect_gp = gpar(type = "none"), cell_fun=cell_fun,column_title = "IFNAR1-/-", column_title_gp = gpar(fontsize = 20, fontface = "bold"))
```

```{r}
f3 = colorRamp2(seq(min(L.matrix), max(L.matrix), length = 3), c("#3c17fe", "#EEEEEE", "#b2312c"))
Heatmap(L.matrix,show_column_dend = FALSE, col = f1,show_column_names=F,show_row_names = T, cluster_col=F, cluster_rows = F, heatmap_legend_param = list(title = ""),top_annotation = ha, rect_gp = gpar(type = "none"), cell_fun=cell_fun,column_title = "IFNLR1-/-", column_title_gp = gpar(fontsize = 20, fontface = "bold"))
